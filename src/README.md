...
```bash
ai-utilities$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ai-utilities$ sam local start-api
ai-utilities$ curl http://localhost:3000/
```
...
```bash
ai-utilities$ sam logs -n HelloWorldFunction --stack-name ai-utilities --tail
```
...
```bash
sam delete --stack-name ai-utilities
```
